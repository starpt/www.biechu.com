	...
	for index, value in ipairs(ActionCommandList) do
		...
		while ( cmdString ) do
			...
			if ( cmdString == command ) then
				...
				return (SecureCmdItemParse(action));
			end
			i = i + 1;
...
local function GetActionFromMacroLines(...)
	local action;
	for i=1, select("#",...) do
		action = GetActionFromMacroLine(select(i,...));
		if ( action ) then
			return action;
		end
	end
...
local function UpdateDynamicMacroFeedback(...)
	...
	for i=1, select("#",...) do
		macro = select(i,...);
		action = GetActionFromMacroText(GetMacroBody(macro));
		if ( action ) then
			if ( GetItemInfo(action) ) then
				SetMacroItem(macro, action);
			else
				SetMacroSpell(macro, action);
			end
		end
	end
end
