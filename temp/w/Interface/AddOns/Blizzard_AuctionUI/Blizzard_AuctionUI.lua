	...
	for i=1, NUM_BROWSE_TO_DISPLAY do
		...
		if ( index > (numBatchAuctions + (NUM_AUCTION_ITEMS_PER_PAGE * AuctionFrameBrowse.page)) ) then
			...
			if ( numBatchAuctions < NUM_BROWSE_TO_DISPLAY ) then
				getglobal(button:GetName().."BuyoutText"):SetPoint("LEFT", getglobal(button:GetName().."BuyoutMoneyFrame"), "RIGHT", BUYOUT_TEXT_OFFSET_NO_SCROLL, 0)
				button:SetWidth(625);
			...
			elseif ( numBatchAuctions == NUM_BROWSE_TO_DISPLAY and totalAuctions <= NUM_BROWSE_TO_DISPLAY ) then
				...
				getglobal(button:GetName().."BuyoutText"):SetPoint("LEFT", getglobal(button:GetName().."BuyoutMoneyFrame"), "RIGHT", BUYOUT_TEXT_OFFSET_WITH_SCROLL, 0)
				button:SetWidth(600);
